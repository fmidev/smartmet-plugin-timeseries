  ...
  try
  {
    ...
    if (geomId != nullptr || coordinateType == nullptr)
    {
      gridQuery.mAttributeList.addAttribute(
          "contour.coordinateType", Fmi::to_string(T::CoordinateTypeValue::GRID_COORDINATES));
    }
          ...
          for (uint x = 0; x < xLen; x++)
          {
            ...
            if (coordinates.empty())
            {
              ...
              if (len > 0)
              {
                ...
                {
                  T::GridValue val;
                  if (gridQuery->mQueryParameterList[p]
                          .mValueList[x]
                          ->mValueList.getGridValueByIndex(v, val))
                  {
        ...
        for (int pp = 0; pp < pLen; pp++)
        {
               ...
               QueryServer::QueryParameter::Flags::AlternativeParameter) == 0)
          {
                ...
                gridQuery->mQueryParameterList[pp].mValueList.size() > 0)
            {
              ...
              {
                if (!gridQuery->mQueryParameterList[pp].mValueList[x]->mValueData.empty())
                  xLen = 1;
            ...
            if (vLen == 0 && ai > 0)
            {
              ...
              vLen = 0;
              xLen = 0;
              if (C_INT(gridQuery->mQueryParameterList.size()) > pid &&
                  !gridQuery->mQueryParameterList[pid].mValueList.empty())
              {
                // ### Going through all timesteps.
                ...
                for (uint x = 0; x < timestepCount; x++)
                {
                  if (!gridQuery->mQueryParameterList[pid].mValueList[x]->mValueData.empty())
                    xLen = 1;

                  uint vv =
                      gridQuery->mQueryParameterList[pid].mValueList[x]->mValueList.getLength();
                  if (vv > vLen)
                  {
                    vLen = vv;
                  }
            ...
            if (vLen > 0 && rLen <= tLen)
            {
              ...
              for (uint v = 0; v < vLen; v++)
              {
                ...
                for (uint t = 0; t < tLen; t++)
                {
                  ...
                  bool res = gridQuery->mQueryParameterList[pid]
                                 .mValueList[t]
                                 ->mValueList.getGridValueByIndex(v, val);
                  ...
                  {
                    res = gridQuery->mQueryParameterList[ai]
                              .mValueList[t]
                              ->mValueList.getGridValueByIndex(v, val);
                    if (res)
                ...
                {
                  T::GridValue val;
                  if (gridQuery->mQueryParameterList[pid]
                          .mValueList[0]
                          ->mValueList.getGridValueByIndex(v, val))
                  {
            ...
            if (!processed && coordinates.size() > 1)
            {
              ...
              while (cname[aaa] != nullptr &&
                     strcasecmp(gridQuery->mQueryParameterList[pid].mParam.c_str(), cname[aaa]) !=
                         0)
                aaa++;
            ...
            if (!processed)
            {
                   ...
                   ++ft)
              {
                ...
                {
                  size_t size =
                      gridQuery->mQueryParameterList[pid].mValueList[t]->mValueData.size();
                  uint step = 1;
                  if (size > 0)
                    step = 256 / size;
                  ...
                  int height =
                      Fmi::stoi(gridQuery->mAttributeList.getAttributeValue("grid.height"));
                  bool rotate = (bool)Fmi::stoi(gridQuery->mAttributeList.getAttributeValue(
                      "grid.original.reverseYDirection"));
                ...
                {
                  auto opt_idx =
                      Fmi::stoi_opt(gridQuery->mQueryParameterList[pid].mParam.substr(3));
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen)
                  {
                    ...
                    {
                      TS::TimedValue tsValue(queryTime,
                                             C_INT(gridQuery->mQueryParameterList[cpid->second]
                                                       .mValueList[t]
                                                       ->mParameterLevel));
                      tsForNonGridParam->emplace_back(tsValue);
                    }
                ...
                else if (gridQuery->mQueryParameterList[pid].mParam == "level")
                {
                  ...
                  while (idx < pLen && levelValue <= 0)
                  {
                    if (gridQuery->mQueryParameterList[idx].mValueList[t]->mParameterLevel > 0)
                    {
                      ...
                      if (levelValue < 0)
                        if (gridQuery->mQueryParameterList[idx].mValueList[t]->mParameterLevelId ==
                            2)
                ...
                {
                  auto opt_idx =
                      Fmi::stoi_opt(gridQuery->mQueryParameterList[pid].mParam.substr(4));
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen)
                  {
                    auto cpid = pidList.find(((ulonglong)(*opt_idx) << 32) + t);
                    if (cpid != pidList.end())
                    {
                      TS::TimedValue tsValue(
                          queryTime,
                          static_cast<int>(gridQuery->mQueryParameterList[cpid->second]
                                               .mValueList[t]
                                               ->mParameterLevelId));
                      tsForNonGridParam->emplace_back(tsValue);
                    }
                ...
                else if (gridQuery->mQueryParameterList[pid].mParam.substr(0, 3) == "@P-")
                {
                  ...
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen)
                  {
                    ...
                    if (cpid != pidList.end())
                    {
                      T::ProducerInfo producer;
                      if (itsGridEngine->getProducerInfoById(
                              gridQuery->mQueryParameterList[cpid->second]
                                  .mValueList[t]
                                  ->mProducerId,
                              producer))
                      {
                      ...
                      else
                      {
                        TS::TimedValue tsValue(
                            queryTime,
                            Fmi::to_string(gridQuery->mQueryParameterList[cpid->second]
                                               .mValueList[t]
                                               ->mProducerId));
                        tsForNonGridParam->emplace_back(tsValue);
                      }
                  ...
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen)
                  {
                    ...
                    if (cpid != pidList.end())
                    {
                      ...
                      bool res = itsGridEngine->getGenerationInfoById(
                          gridQuery->mQueryParameterList[cpid->second].mValueList[t]->mGenerationId,
                          info);
                      if (res)
                      {
                  ...
                  while (idx < pLen)
                  {
                    if (gridQuery->mQueryParameterList[idx].mValueList[t]->mGenerationId > 0)
                    {
                      ...
                      if (res)
                      {
                        ...
                        boost::local_time::local_date_time origTime(toTimeStamp(info.mAnalysisTime),
                                                                    tz);

                        // boost::local_time::local_date_time origTime(oTime);
                        TS::TimedValue tsValue(queryTime,
                                               masterquery.timeformatter->format(origTime));
                        tsForNonGridParam->emplace_back(tsValue);
                        idx = pLen + 10;
                      }
                    }
                ...
                else if (gridQuery->mQueryParameterList[pid].mParam.substr(0, 3) == "@I-")
                {
                  ...
                  if (opt_idx && *opt_idx >= 0 && *opt_idx < pLen)
                  {
                    ...
                    if (cpid != pidList.end())
                    {
                      ...
                      T::ProducerInfo producer;
                      if (itsGridEngine->getProducerInfoById(
                              gridQuery->mQueryParameterList[i].mValueList[t]->mProducerId,
                              producer))
                ...
                else if (rLen > tLen)
                {
                  ...
                  for (uint r = 0; r < rLen; r++)
                  {
                    if (gridQuery->mQueryParameterList[pid].mValueList[r]->mForecastTimeUTC == *ft)
                    {
                      ...
                      T::ProducerInfo producer;
                      if (itsGridEngine->getProducerInfoById(
                              gridQuery->mQueryParameterList[pid].mValueList[r]->mProducerId,
                              producer))
                        producerName = producer.mName;
