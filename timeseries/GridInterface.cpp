...
void GridInterface::prepareQueryTimes(QueryServer::Query& gridQuery,const Query& masterquery,const Spine::LocationPtr& loc)
{
  ...
  try
  {
    ...
    if (masterquery.toptions.startTimeData ||
        masterquery.toptions.endTimeData ||
...
void GridInterface::prepareProducer(
...
void GridInterface::prepareLocation(QueryServer::Query& gridQuery,const Query& masterquery,const Spine::LocationPtr& loc,
    const T::GeometryId_set& geometryIdList,std::vector<std::vector<T::Coordinate>>& polygonPath,uchar& locationType)
{
  FUNCTION_TRACE
  try
  {
...
)
{
  ...
  try
  {
    // Converting parameter information into parameter objects that are used by the grid-engine/QueryServer.
    ...
    std::vector<std::vector<T::Coordinate>>& polygonPath)
{
  ...
  try
  {
    ...
    prepareQueryParameters(gridQuery,masterquery,mode,levelId,geometryId,locationType,
        sameParamAnalysisTime,origLevel,areaproducers);
  }
  catch (...)
  {
    throw Fmi::Exception(BCP, "Operation failed!", nullptr);
  ...
  try
  {
    ...
    for (int pp = 0; pp < pLen; pp++)
    {
      ...
      if ((gridQuery->mQueryParameterList[pp].mFlags & QueryServer::QueryParameter::Flags::AlternativeParameter) == 0)
      {
        ...
        // If "processed" variable is false then the query has returned no values for the parameter. This
        // usually means that the parameter is not a data parameter. It is most likely "a special parameter" that
...
*/
            ...
            else if (UtilityFunctions::is_special_parameter(gridQuery->mQueryParameterList[pid].mParam))
            {
              ...
              {
                TS::Value pValue = TS::time_parameter(paramname, queryTime, state.getTime(), *loc, timezone, itsTimezones,
                    masterquery.outlocale, *masterquery.timeformatter,masterquery.timestring);
              ...
              {
                TS::Value pValue = TS::location_parameter(loc, paramname, masterquery.valueformatter, timezone,
                    gridQuery->mQueryParameterList[pid].mPrecision,masterquery.crs);
                TS::TimedValue tsValue(queryTime, pValue);
                tsForNonGridParam->emplace_back(tsValue);
              }
            }
            else if (additionalParameters.getParameterValueByLocation(gridQuery->mQueryParameterList[pid].mParam,
                tloc.tag, loc, country,gridQuery->mQueryParameterList[pid].mPrecision, paramValue))
            {
              // ################################################################################
            ...
            else if (additionalParameters.getParameterValueByLocationAndTime(gridQuery->mQueryParameterList[pid].mParam,
                tloc.tag, loc, queryTime, tz,gridQuery->mQueryParameterList[pid].mPrecision, paramValue))
            {
              // ################################################################################
            ...
            else if (gridQuery->mQueryParameterList[pid].mParam.substr(0,1) == "@")
            {
              ...
              //
              //    @P-0    => The producer name of the first parameter in the result (index starts from zero).
  ...
  try
  {
    ...
    for (uint mode = 0; mode < 3; mode++)
    {
      ...
      for (const auto level : levels)
      {
        ...
        prepareGridQuery(*originalGridQuery, masterquery, mode, qLevelId, level, areaproducers, tloc, loc, geometryIdList, polygonPath);
        ...
        exteractQueryResult(gridQuery,state,masterquery,outputData,queryStreamer,areaproducers,tz,tloc,loc,country,level);
      }
    }
  }
  catch (...)
